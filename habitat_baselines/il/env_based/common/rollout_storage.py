...
class RolloutStorage:
    ...
    def recurrent_generator(self, num_mini_batch, advantages=None):
        if advantages is None:
            return self.recurrent_generator_il(num_mini_batch)
        else:
            return self.recurrent_generator_ppo(advantages, num_mini_batch)


    def recurrent_generator_il(self, num_mini_batch):
    ...
    def recurrent_generator_ppo(self, advantages, num_mini_batch):
        num_processes = self.rewards.size(1)
        assert num_processes >= num_mini_batch, (
            "Trainer requires the number of processes ({}) "
            "to be greater than or equal to the number of "
    ...
    def compute_returns(self, next_value, use_gae=True, gamma=0.99, tau=0.95):
        if use_gae:
