...
class ObjectNavILSimpleNet(Net):
    ...
    def __init__(self, observation_space: Space, model_config: Config, num_actions, device=None):
        ...
        assert model_config.VISUAL_ENCODER.cnn_type in [
            "VisualResnetEncoder",
            "None",
        ], "VISUAL_ENCODER.cnn_type must be VisualResnetEncoder"
        if model_config.VISUAL_ENCODER.cnn_type == "VisualResnetEncoder":
            self.visual_encoder = VisualResnetEncoder(
                observation_space,
                output_size=model_config.VISUAL_ENCODER.output_size,
                backbone=model_config.VISUAL_ENCODER.backbone,
                trainable=model_config.VISUAL_ENCODER.trainable,
            )
            rnn_input_size += model_config.VISUAL_ENCODER.output_size
        else:
